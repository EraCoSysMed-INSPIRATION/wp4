...
inputFile <- "indiv_results_cov_bobyqa_init20.xlsx"
...
for (i in seq_along(myEst)){
  ...
  lim <- max(ggplot_build(plot)$data[[1]]$count)*1.4
   plot <- plot + geom_vline(aes_(xintercept=rv), colour="black", linetype="dashed", size=1) +
