...
file.sources = list.files(file.path(getwd(),"../../OSPSuite.ParameterIdentification-develop/R"), 
...
for (i in 1:length(ds)) {
  addWorksheet(indiv_theo, sheetName=names(ds[i]))
  writeData(indiv_theo, sheet=names(ds[i]), x=ds[[i]]) # Note [[]]
}
...
for(i in names(ds)){
  ...
  piOutputMapping$addObservedData(observedData[[i]][[i]])
...
write.xlsx(output, file.path(dataFolder,"indiv_results.xlsx"), overwrite = TRUE)
